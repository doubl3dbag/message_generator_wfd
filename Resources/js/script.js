...
//meal object to hold -----
//5 different meal options for each ethnicity for variety
//what meal was had the previous day and what meal will be for current day
const mealObj = {
    ...
    getRandomEthnicity() {
        ...
        this._meal._ethnicity.push(ethnicities[randomEhnicity]);
    },
    getRandomCourse() {
        const courses = this._meal._ethnicity;
        if (courses === "Chinese") {
            this._meal._meal.push(this._courses.Chinese.length);
        }
        else if (courses === "Mexican") {
            this._meal._meal.push(this._courses.Mexican.length);
        }
        else if (courses === "American") {
            this._meal._meal.push(this._courses.American.length);
        }
        else if (courses === "Italian") {
            this._meal._meal.push(this._courses.Italian.length);
        }
        else if (courses === "Greek") {
            this._meal._meal.push(this._courses.Greek.length);
        }
        else if (courses === "French") {
            this._meal._meal.push(this._courses.French.length);
        }
        else if (courses === "Thai") {
            this._meal._meal.push(this._courses.Thai.length);
        }
        else if (courses === "Indian") {
            this._meal._meal.push(this._courses.Indian.length);
        }
        return courses;
    }
    // addMealToMeal() {
    //     let courseHolder = this._meal._ethnicity;
    //     let course = courseHolder;
    //     if(course === "Chinese") {
    ...
    //     this._meal._ethnicity.push(ethnicities[randomEhnicity]);
    // },
    // addMealToMeal(ethnicity) {
    //     const mealEthnicity = this._meal._ethnicity;
    //     const ethnicMeals = this._courses[ethnicity];
    //     const randomEthnicMeal = randomizeNum(ethnicMeals.length);
    //     if (mealEthnicity === "Chinese") {
    //         this._meal._meal
    //     }
    // }
    // addMealToCourse(mealEthnicity, mealName, mealDiscription, mealRecipe) {
    //     const meal = {
    //         name: mealName,
    //         discription: mealDiscription,
    //         recipe: mealRecipe,
    //     };
    //     this._ethnicities[mealEthnicity].push(meal);
    // },
};

mealObj.getRandomEthnicity();
console.log(mealObj.getRandomCourse());
console.log(mealObj._meal)