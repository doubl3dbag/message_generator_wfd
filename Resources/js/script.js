...
function placeMeal() {
    ...
    if(counter === 1) {
        prevMeals._day._monday.pop();
        prevMeals._day._monday.push(mealHolder);
    }
    else if(counter === 2) {
        prevMeals._day._tuesday.pop();
        prevMeals._day._tuesday.push(mealHolder);
    }
    else if(counter ===3) {
        prevMeals._day._wednesday.pop();
        prevMeals._day._wednesday.push(mealHolder);
    }
    else if(counter === 4) {
        prevMeals._day._thursday.pop();
        prevMeals._day._thursday.push(mealHolder);
    }
    else if(counter === 5) {
        prevMeals._day._friday.pop();
        prevMeals._day._friday.push(mealHolder);
    }
    else if(counter === 6) {
        prevMeals._day._saturday.pop();
        prevMeals._day._saturday.push(mealHolder);
    }
    else {
        prevMeals._day._sunday.pop();
        prevMeals._day._sunday.push(mealHolder);
    }
    return mealHolder;
}
...
function getRecipie() {
    ...
    let mealHold = [];
       if(counter === 1) {
            mealHold = prevMeals._day._monday;
...
// function getRecipie() {
//     let recipeHolder = [];
//     for (let recipe in mealTypes) {
//         let mealRecipe = randomizeNum(mealTypes[recipe].length);

//         switch (recipe) {
//             case 'Chinese':
//                 recipeHolder.push(`${mealTypes[recipe][mealRecipe]}`)
//                 break
//             case 'Mexican':
//                 recipeHolder.push(`${mealTypes[recipe][mealRecipe]}`)
//                 break
